...
def windows(p,title,mode):
 n=['black','red','green','yellow','blue','purple','cyan','white']; d={"name":f"Neon Mary: {title} {mode}","background":p['bg'],"foreground":p['fg'],"cursorColor":p['accent']}; d.update(dict(zip(n,p['c'][:8]))); d.update(dict(zip(['bright'+x.title() for x in n],p['c'][8:]))); return json.dumps(d,indent=2)+"\n"
